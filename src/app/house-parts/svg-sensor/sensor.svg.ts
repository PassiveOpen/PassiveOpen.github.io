import { SafeHtml } from "@angular/platform-browser";
import * as d3 from "d3";
import { angleBetween, angleXY, round } from "src/app/shared/global-functions";
import { HousePartSVG } from "../model/housePart.model";
import { CableType, Floor, SensorType } from "src/app/components/enum.data";
import { Wall } from "../wall.model";
import { xy } from "src/app/house/house.model";

export class SensorSVG<T> extends HousePartSVG {
  svgG: d3.Selection<SVGGElement, unknown, HTMLElement, any>;
  svgIcon: d3.Selection<SVGGElement, unknown, HTMLElement, any>;
  svgBadge: d3.Selection<SVGGElement, unknown, HTMLElement, any>;
  svgBadgeCircle: d3.Selection<SVGCircleElement, unknown, HTMLElement, any>;
  svgBadgeText: d3.Selection<SVGTextElement, unknown, HTMLElement, any>;

  setD3(): void {
    if (this.floor === undefined || this.floor in this.model.parent) {
      this.floor = (this.model.parent as any).floor;
    }
    this.svg = d3.select(`#${this.selector}.sensor-cable`);
    if (this.model.sensorType === SensorType.temperature) {
      this.model.sensorOnly = true;
    }
    if (!this.model.cableOnly) {
      // So it has a Icon
      this.svgG = d3.select(`#${this.selector}.sensor-g`);
      this.model.showBadge = this.model.amount >= 2 && this.svgG !== undefined;
      this.svgIcon = d3.select(`#${this.selector}.sensor-icon`);
      if (this.model.showBadge) {
        this.svgBadge = this.svgG.append("g");
        this.svgBadgeCircle = this.svgBadge.append("circle");
        this.svgBadgeText = this.svgBadge.append("text");
      }
    } else {
      this.lineThickness = 2;
      this.classes.push("sensor-room-cable");
    }
    if (this.model.group) this.classes.push(`el-group-${this.model.group}`);
    if (this.model.cableType)
      this.classes.push(`cable-${this.model.cableType}`);
    this.classes.push(`sensor-${this.model.sensorType}`);
  }
  drawWhenNotVisible(): void {
    if (this.svgIcon) this.svgIcon.attr("xlink:href", "");
    if (this.svg) this.svg.attr("points", "");
    if (this.svgBadgeCircle) this.svgBadgeCircle.attr("r", 0);
    if (this.svgBadgeText) this.svgBadgeText.attr("font-size", 0);
    this.model.showBadge = false;
  }
  initDraw(): void {
    if (this.svgIcon) {
      this.svgIcon.attr(
        "xlink:href",
        `#icon-${this.getSensorType(this.selector)}`
      );
    }
    const sensorPoint = this.calcOffsetWall();

    if (!this.model.cableOnly) {
      // this.svgIcon.attr(
      //   "transform",
      //   `translate(${sensorPoint[0]}, ${sensorPoint[1]}) scale(${
      //     this.show(floor) ? 1 / 100 : 1 / 10000
      //   })`
      // );

      this.svgIcon
        .attr("x", `${sensorPoint[0]}px`)
        .attr("y", `${sensorPoint[1]}px`);
      // console.log(this.svgIcon);

      this.setClass(this.svgG);
      this.setClass(this.svg);
    }

    if (this.model.showBadge) {
      this.badge(sensorPoint);
    } else {
      // if (this.selector === "L0-West-EastWall-light-switch-1")
      //   console.log(2, this);
      if (this.svgBadgeCircle) this.svgBadgeCircle.attr("r", 0);
      if (this.svgBadgeText) this.svgBadgeText.text(``);
    }

    if (!this.model.sensorOnly) {
      this.svg
        .attr("points", this.cablePoints(sensorPoint, this.floor).join(" "))
        .attr("transform", this.transform);
      this.setClass(this.svg);
      this.model.cableLength = this.getLength();
    }
  }
  updateScale(): void {
    if (this.svgBadge) {
      this.svgBadge
        .attr("stroke-width", this.meterPerPixel * this.lineThickness * 1)
        .attr(
          "transform",
          `translate(${this.meterPerPixel * 10} ${this.meterPerPixel * -10})`
        );
      this.svgBadgeCircle.attr("r", this.meterPerPixel * 8);

      this.svgBadgeText
        .attr("font-size", this.meterPerPixel * this.model.fontSize)
        .text(`${this.model.amount}`);
    }
    if (this.svg) {
      this.svg.attr(
        "stroke-width",
        this.meterPerPixel * this.lineThickness * 1
      );
      if (this.model.cableType === CableType.OutsidePOE)
        this.svg.attr(
          "stroke-dasharray",
          `${this.meterPerPixel * this.lineThickness * 10 * 0.5} ${
            this.meterPerPixel * this.lineThickness * 10
          }`
        );
    }
  }

  getSensorType = (str: string) => {
    const a = Object.values(SensorType).find((y) => str.includes(y));
    return a;
  };
  cablePoints(sensorPoint: number[], floor: Floor): number[][] {
    return this.show(floor) ? [sensorPoint, ...this.model.points] : [];
  }

  getLength() {
    const dist = (p1, p2) => Math.hypot(p2[0] - p1[0], p2[1] - p1[1]);
    let length = 0;

    this.model.points.forEach((p, i, arr) => {
      if (i + 1 === this.model.points.length) return;
      length += dist(p, arr[i + 1]);
    });
    return round(
      length + this.model.verticalCableLength + this.model.amount * 0.3
    );
  }

  badge(coords: xy) {
    this.svgBadge.attr("class", "sensor-badge");

    this.svgBadgeCircle
      .attr("fill", this.model.amount === 2 ? "white" : "#333")
      .attr("cx", coords[0])
      .attr("cy", coords[1])
      .attr("stroke", "black");

    this.svgBadgeText
      .attr("fill", this.model.amount === 2 ? "black" : "white")
      .attr("x", coords[0])
      .attr("y", coords[1])
      .attr("text-anchor", "middle")
      .attr("class", "sensor-badge-text")
      .attr("stroke", "none")
      .attr("dominant-baseline", "middle")
      .text(`${this.model.amount}`);
  }

  calcOffsetWall(): xy {
    if (this.model.parent instanceof Wall) {
      const wall = this.model.parent as Wall;

      const arr = wall.sides[this.model.wallSide];
      const angle = angleBetween(arr[0], arr[1]);
      return angleXY(angle + 90, this.model.offsetWall, this.model.points[0]);
    } else {
      return [
        this.model.points[0][0] + this.model.offset[0],
        this.model.points[0][1] + this.model.offset[1],
      ];
    }
  }

  tooltip = (): SafeHtml => {
    // const [all, type, subtype, level, roomCode, side] =
    //   /.(\w+)-(\w+)-(\d)-(\w+)-(\w+)/gi.exec(this.selector); // sensor-socket-0-f-n-2

    let text = `<b>${this.model.sensorType.toTitleCase()} `;
    // ${this.}-${side}</b> (+${level})`

    if (this.model.cableLength > 0) {
      text += `<br>Cable length ${round(this.model.cableLength, 1)}m2`;
    }
    if (this.model.group) {
      text += `<br>Group ${this.model.group}`;
    }

    return text;
  };

  select() {
    document.querySelectorAll("svg .selected").forEach((d, i) => {
      d.classList.remove("selected");
    });

    if (
      [SensorType.socket, SensorType.perilex].includes(this.model.sensorType)
    ) {
      document
        .querySelectorAll(`.el-group-${this.model.group}`)
        .forEach((d, i) => {
          d.classList.add("selected");
        });
    } else {
      this.svg.node().classList.add("selected");
      this.svgIcon.node().classList.add("selected");
    }
  }
}
