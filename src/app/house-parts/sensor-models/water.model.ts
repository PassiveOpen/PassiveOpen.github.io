...
export class Water<T> extends Sensor<T> {
  ...
  afterD3() {
    const ceiling = this.parent.house.cross.ceilingHeight;
    const floorThickness = this.parent.cross.topFloorThickness;

    if (this.sensorType === SensorType.toilet) {
      this.size = 120;
      this.elevation = 0;
      this.verticalCableLength +=
        this.floor === Floor.top
          ? ceiling + floorThickness * 2
          : floorThickness;
